...
const { domainApiUrl,ajaxPrefix, ajaxTimeout } = cfg;
/*
* 菜单数据 返回Promise各式，支持前端硬编码、异步获取菜单数据
* */
export default function getMenus() {
    let menuUrl = domainApiUrl+ajaxPrefix+'/biz/menu/userMenu';
    return ajax.post(menuUrl, {}).then(res => {
        let menuData =  (res || []).map(item => (
            {key: ''+item.menuId, text:item.menuName,icon:item.icon,
                path:item.url,parentKey: ''+item.parentId}));
