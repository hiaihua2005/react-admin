...
export default class index extends Component {
    ...
    columns = [
        {
            title: '名称', dataIndex: 'menuName', key: 'menuId',
            render: (value, record) => {
                const { icon } = record;
        ...
        {
            title: '类型', dataIndex: 'menuType', key: 'menuId',
            render: (value, record) => {
                const { url } = record;
                if(url) {
                    if (value === 'M') return '菜单目录';
    ...
    handleSearch() {
        ...
        this.props.ajax
            .post('/biz/menu/list')
            .then(res => {
                const menus = res.map(item => ({ key: item.menuId, parentKey: item.parentId, ...item }));
